...
namespace MatNETIO.types
{
    ...
    public class MLArray
    {
		...
		/// </summary>
		/// <comments>Primary dimension, use with caution.</comments>
        ...
        /// <comments>Secondary dimension, use with caution.</comments>
		public int N
        {
            get
            {
        ...
        /// <summary>
		/// Get the N (number of) dimensions or the size of the dimensions array
		/// </summary>
        ...
        public int Size
        {
            get
            {
                ...
                return _dims.Aggregate(1, (current, t) => current*t);
            }
        }
        ...
        /// <remarks>Only apply to 2D array. Use with caution.</remarks>
        ...
        /// <summary>
        /// Get the index of a subscription.
        /// </summary>
        /// <param name="subscripts">Subscription [i, j, k, ...] (in matlab convension) of an element. </param>
        /// <returns>An column-packed index into the array (in C# convension: starting from 0).</returns>
        /// <remarks>
        /// Subscripts should has the same dimension with the array.
        /// For example, Subscript [2 1 2] of a 2x3x2 array should return 7.
        /// (2-1)*1 + (1-1)*2 + (2-1)*2x3 = 7
        /// </remarks>
        protected int GetIndexOfMatSubs(int[] subscripts)
        {
            if(subscripts.Length < 2)
                throw new MatlabIOException("Input subscript length should be greater than 1!");

            if(_dims.Length != subscripts.Length)
                throw new MatlabIOException("Input subscript dimension not match with the array!");

            for (int i = 0; i < _dims.Length; i++)
            {
                if (subscripts[i] > _dims[i])
                    throw new MatlabIOException("Out of index of dimension " + i + ": " + _dims[i]);
            }
        ...
        /// <summary>
        /// Get the index of a subscripts in C# convention.
        /// </summary>
        /// <param name="subscripts">Subscription [i, j, k, ...] (in c# convension) of an element.</param>
        /// <returns>An index into the array (in C# convension: starting from 0).</returns>
        /// <remarks>
        /// Subscripts should has the same dimension with the array.
        /// For example, Subscript [1 0 1] (c# convention) of a 2x3x2 array should return 7.
        /// 1*1 + 0*2 + 1*2x3 = 7
        /// </remarks>
        protected int GetIndex(int[] subscripts)
        {
            if (subscripts.Length < 2)
                throw new MatlabIOException("Input subscript length should be greater than 1!");
